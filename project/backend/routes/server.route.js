...
const User = require("../models/User.model"); // Đảm bảo đường dẫn trỏ đúng tới model User

// Import các route con
const artistsRoute = require("./artists.route");
const adminRoute = require("./admin.route");

// --- CÁC ROUTE AUTH (Login/Register) ---
...
router.post("/login", (req, res) => {
  User.findOne({ email: req.body.email })
    .then((user) => {
      if (user) {
        bcrypt.compare(req.body.password, user.password, (err, matched) => {
          if (err)
            return res.json({ success: false, message: "Error comparing passwords", error: err.message });
          if (matched) {
    ...
    .catch((err) => {
      res.json({ success: false, message: "Error finding user", error: err.message });
    });
});

// --- GẮN CÁC ROUTE CON ---
// Lưu ý: Ở đây mình gắn prefix /api/artists và /api/admin
